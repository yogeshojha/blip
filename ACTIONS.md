...
A pack is a directory under `~/.config/omarchy/blip/packs/` with the same
`actions/` and `scripts/` layout:

```bash
git clone https://github.com/someone/blip-pack-devtools \
  ~/.config/omarchy/blip/packs/devtools
```
